...
const Temp = () => {
  ...
  const keyHandler = (e) => {
    switch (e.key) {
      case "ArrowUp":
        if (ladders.includes(left) && bottom < 200) climbLadder(10);
        break;
      case "ArrowDown":
        if (ladders.includes(left) && bottom > 0) climbLadder(-10);
        break;
      case "ArrowLeft":
        moveCharacter(-10);
        break;
  ...
  useEffect(() => {
    if (!falling) window.addEventListener("keydown", keyHandler);
  ...
  useEffect(() => {
    //gravity
    let isFloorPresent = floors.find(
      (e) => e.left <= left && e.left + e.width > left && bottom === floorHeight
    );
  ...
  const moveBullets = () => {
    bulletTimer.push(
      setTimeout(() => {
        ...
        newBullets = newBullets.map((e) => ({
          ...e,
          left: e.left + e.direction,
        }));
        console.error(newBullets);
        newBullets = newBullets.filter(
          (e) => e.left > camera && e.left < camera + frameSize
        );
        console.error(newBullets);
  ...
  const isInsideFrame = (start, end) =>
    end > camera && start < camera + frameSize;
  return (
    <div>
      <Grid border={"black"} color={"#4f5382"}>
        <div id="ladders">
          {ladders.map((left) =>
            isInsideFrame(left, left + 20) ? (
              <Ladder left={left - camera}>
                <img src={LadderImg} />
              </Ladder>
            ) : (
              ""
        ...
        <div id="floors">
          {floors.map(({ left, width }) =>
            isInsideFrame(left, left + width) ? (
              <Floor left={left - camera} width={width} />
            ) : (
              ""
            )
          )}
        ...
        <div id="bullets">
          {bullets.map(({ left, bottom }) => (
            <Bullets left={left - camera} bottom={bottom} />
          ))}
        </div>
