...
box-shadow: 0px 1px 41px 20px ${({reverse})=>reverse?'#0075ff36':'#ff000036'} inset;
`
...
let fallTimer = null
const Tenet = () => {
    const [ceiling, setCeiling] = useState([...Array(6)].map((e, i) => ({ id: i, bottom: 248, left: (i + 2) * 20 })));
    let floor = [...Array(20)].map((e, i) => ({ id: `floor${i}`, bottom: 248, left: (i) * 20, floor: true }));
    ...
    useEffect(() => {
        ...
        if (fall && reverse === false && ceiling.find(e => e.bottom)) {
            gravityTimer = setTimeout(() => {
                ...
                newCeiling = newCeiling.map(e => e.broke ? { ...e, bottom: ((e.bottom - 8) >= 0 ? e.bottom - 8 : 0) } : e)
    ...
    useEffect(() => {
        ...
        if (!fall && reverse && ceiling.find(e => e.bottom < 248)) {
            ...
            antiGravityTimer = setTimeout(() => {
                ...
                newCeiling = newCeiling.map(e => !e.broke ? { ...e, bottom: ((e.bottom + 8) <= 248 ? e.bottom + 8 : 248) } : e)
                let topBrick = newCeiling.filter(e => e.bottom === 248 && e.rotate).pop()
                if (topBrick) {
                    topBrick.rotate = 0
                }
                setCeiling(newCeiling)
    ...
    useEffect(() => {
        if (fall && !reverse && bottom) {
            let fallAmount = 8
            let floorPresent = floor.find(e => (e.bottom + fallAmount) === bottom && [left - 10, left].includes(e.left))
    ...
    useEffect(() => {
        if (!fall && reverse && bottom <= 248) {
            let fallAmount = 8
            let floorPresent = ceiling.find(e => [bottom, bottom + fallAmount].includes(e.bottom) && [left - 10, left].includes(e.left))
            if (floorPresent) {
    ...
    const playerHandler = (e) => {
        ...
        switch (e.key) {
            case 'ArrowRight': {
                if (left + moveAmount === 360 && bottom === 256) setGameOver('SUCCESS !!!')
                if (left + moveAmount === 360 && bottom === 0) setReverse(prev => !prev)
    ...
    const resetGame = () => {
        setCeiling([...Array(6)].map((e, i) => ({ id: i, bottom: 248, left: (i + 2) * 20 })));
        setReverse(false);
        setBottom(256)
        setLeft(0)
        setFall(false)
    ...
    return (
        <div>
            ...
            <Grid border={'black'}>
                {[...ceiling, ...floor].map(({ id, bottom, left, rotate, floor }, i) => (
                    <Ceiling key={id}
                        bottom={bottom}
                        left={left}
                        floor={floor}
                        rotate={rotate} ></Ceiling>))}
                ...
                <Exit src={Door} bottom={exitPosition[0]} left={exitPosition[1]}></Exit>
                <Poster src={ExitImg} />
            </Grid>
            </Container>
