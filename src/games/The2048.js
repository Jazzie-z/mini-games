...
const The2048 = () => {
    ...
    const keyHandler = (e) => {
        switch (e.key) {
            case "ArrowRight": addAndMoveHorizontally(1)
                break;
            case "ArrowLeft": addAndMoveHorizontally(-1)
                break;
            case "ArrowUp": addAndMoveVertically(-1)
                break;
            case "ArrowDown": addAndMoveVertically(1)
                break;
        }
    }
    useEffect(() => {
    ...
    return (
        <div>
            <CustomGrid color={'#bdad9e'}>
                {squares.map((value, i) => <Block key={i} color={colorMap[value]} value={value}>{value || ''}</Block>)}
            </CustomGrid>
